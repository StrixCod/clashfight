...
// ════════════════════════════════════════
// ACTION : score_clash_bot
// Permet au JS de noter un clash côté serveur
// sans l'enregistrer en base (mode bot local)
// ════════════════════════════════════════
// (géré dans le switch ci-dessous)
...
function submitClash(int $gid, int $uid, string $content): array
{
    ...
    $score      = $scored['score'];
    if ($isRepeatEnemy) $score = max(0, $score - REPEAT_ENEMY_PENALTY);
    if ($isRepeatOwn)   $score = round($score * REPEAT_OWN_MULT, 1);
...
if ($action) {
    try {
        ...
        switch ($action) {
            ...
            // ── Action dédiée au mode bot : note un clash via Mistral sans l'enregistrer ──
            ...
            // ── Action dédiée au mode bot : génère le clash du bot via Mistral ──
...
Réponds UNIQUEMENT avec le texte du clash, sans guillemets ni explication.";
            ...
            case 'admin_ban':
                ...
                if ($r['ok']) jsonOk(); else jsonError($r['error']);
                break;
            case 'admin_delete':
                requireAdmin(); checkCSRF();
            ...
            case 'check_ban_status':
                $u = currentUser();
                if (!$u) { jsonOk(['banned' => false, 'ip_banned' => false]); break; }
...
// INIT + HTML
// ════════════════════════════════════════
try { initDB(); } catch (Throwable $e) { die('<pre style="color:red">Erreur DB : ' . h($e->getMessage()) . '</pre>'); }

$csrf   = csrf();
$user   = currentUser();
if ($user) db()->prepare("UPDATE users SET is_online=1,last_seen=datetime('now') WHERE id=?")->execute([$user['id']]);
